...
def create_donation():
    ...
    if form.validate_on_submit():
        ...
        if lat is None:
            flash('Não foi possível encontrar as coordenadas para o endereço da doação.')
        else:
            donation = Donation(description=form.description.data, quantity=form.quantity.data, donor=current_user,
...
def dashboard():
    ...
    if current_user.user_type == 'charity':
        claimed_by_me = Donation.query.filter_by(status='claimed', claimed_by_id=current_user.id).all()

    # Doações do utilizador (doador) que foram reservadas por outros
    my_donations_claimed = []
    if current_user.user_type == 'donor':
        my_donations_claimed = Donation.query.filter_by(status='claimed', user_id=current_user.id).all()
...
def generate_static_map_url(donations):
    ...
    if not api_key or not donations:
        return "https://placehold.co/600x400/e2e8f0/64748b?text=Nenhuma+doacao+disponivel"
    base_url = "https://maps.googleapis.com/maps/api/staticmap"
    markers = [f"{d.latitude},{d.longitude}" for d in donations]
    params = {'size': '600x400', 'maptype': 'roadmap', 'markers': 'color:red|' + '|'.join(markers), 'key': api_key}
